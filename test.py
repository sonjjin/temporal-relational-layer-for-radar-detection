...
class TestModule(object):
    ...
    def test(self, args, down_ratio):
        ...
        annopath = os.path.join(args.data_dir, 'test/labelTxt', '{}.txt')
        img_save_path = os.path.join('result_img', args.backbone+'_'+args.train_mode+'_'+args.exp)
        ...
        total_time = []
        for cnt, data_dict in enumerate(tqdm(data_loader)):
            image_cp = data_dict['image_cp'][0].to(self.device)
            image_pc = data_dict['image_pc'][0].to(self.device)
            # image = data_dict['image'][0].to(self.device)
            img_id = data_dict['img_id'][0]
            # print(image_cp.shape)
            b, c, h, w = image_cp.shape
            gt_bbox = self.get_gt_bbox(os.path.join(annopath.format(img_id)), w, h)
            
            # pdb.set_trace()
            # print('processing {}/{} image ...'.format(cnt, len(data_loader)))
            begin_time = time.time()
            with torch.no_grad():
                pr_decs, _ = self.model(image_cp, image_pc)
            ...
            #"""
            ori_image, _ = dsets.load_image(cnt)
            ...
            for cat in dsets.category:
                ...
                for pred in result:
                    ...
                    ori_image = cv2.drawContours(ori_image, [np.int0(box)], -1, (255,0,0),1,1)          
            ...
            for cat in dsets.category:
                ...
                for i in range(len(gt_bbox)):
                    ori_image = cv2.drawContours(ori_image, [np.int0(gt_bbox[i])], -1, (0,0,255),1,1)
                    
                    
            cv2.imwrite(os.path.join(img_save_path, '{}.png'.format(img_id)), ori_image)
            # pdb.set_trace()
    ...
    def get_gt_bbox(self, filename, w, h):
        gt_bbox = []
        with open(filename, 'r') as f:
            for i, line in enumerate(f.readlines()):
                    ...
                    if len(obj)>8:
                        ...
                        gt_bbox.append([[x1,y1], [x2,y2], [x3,y3], [x4,y4]])
