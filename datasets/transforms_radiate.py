...
def random_flip(img_cp, img_pc, gt_pts_cp, gt_pts_pc, crop_center=None):
    # image: h x w x c
    ...
    if np.random.random()<0.5:
        ...
        if gt_pts_cp.shape[0]:
            gt_pts_cp[:,0] = w-1 - gt_pts_cp[:,0]
            gt_pts_cp[:,4] = 360 - gt_pts_cp[:,4]
        
        if gt_pts_pc.shape[0]:
            gt_pts_pc[:,0] = w-1 - gt_pts_pc[:,0]
            gt_pts_pc[:,4] = 360 - gt_pts_pc[:,4]
    ...
    if np.random.random()<0.5:
        ...
        if gt_pts_cp.shape[0]:
            gt_pts_cp[:,1] = h-1 - gt_pts_cp[:,1]
            gt_pts_cp[:,4] = 180 - gt_pts_cp[:,4]
            gt_pts_cp[gt_pts_cp<0] = gt_pts_cp[gt_pts_cp<0] + 360
            
        if gt_pts_pc.shape[0]:
            gt_pts_pc[:,1] = h-1 - gt_pts_pc[:,1]
            gt_pts_pc[:,4] = 180 - gt_pts_pc[:,4]
            gt_pts_pc[gt_pts_pc<0] = gt_pts_pc[gt_pts_pc<0] + 360
        if crop_center is not None:
            crop_center[1] = h-1 - crop_center[1]
    return img_cp, img_pc, gt_pts_cp, gt_pts_pc, crop_center
