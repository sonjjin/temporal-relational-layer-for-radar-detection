...
def decode_prediction(predictions, dsets, args, img_id, down_ratio):
    ...
    ori_image, _ = dsets.load_image(dsets.img_ids.index(img_id))
    ...
    for pred in predictions:
        ...
        score = pred[8]
        clse = pred[9]
        pts = np.asarray([tr, br, bl, tl], np.float32) # 4점좌표 category x 2 x 4
...
def write_results(args,
    ...
    for index in tqdm(range(len(dsets))):
        data_dict = dsets.__getitem__(index)
        image_cp = data_dict['image_cp'].to(device)
        image_pc = data_dict['image_pc'].to(device)
        ...
        with torch.no_grad():
            pr_decs, _ = model(image_cp, image_pc)
        ...
        pts0, scores0 = decode_prediction(predictions, dsets, args, img_id, down_ratio) #[tr, br, bl, tl]
            ...
            for img_id in results[cat]:
                for pt in results[cat][img_id]:
                    f.write('{} {:.12f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.format(
                        img_id, pt[8], pt[0], pt[1], pt[2], pt[3], pt[4], pt[5], pt[6], pt[7])) # img, confidence, tr_x, tr_y, br_x, br_y, bl_x, bl_y, tl_x, tl_y
