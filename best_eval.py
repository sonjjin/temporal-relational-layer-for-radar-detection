...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='/workspace/dataset/radiate_lidar', help='Data directory')
    parser.add_argument('--phase', type=str, default='train', help='Phase choice= {train, test, eval}')
    parser.add_argument('--only_objects', type=bool, default=False, help='Whether to use only datasets with objects')
...
if __name__ == '__main__':
    ...
    mAP_best = 0
    model_weight = np.arange(5, 25, 5)
    result_path = os.path.join('result', args.backbone+'_'+args.train_mode+'_'+args.exp)
    
    for weight in model_weight:
        args.resume = 'model_' + str(weight) + '.pth'
        ctrbox_obj = eval.EvalModule(dataset=dataset, num_classes=num_classes, model=model, decoder=decoder)
        mAP = ctrbox_obj.evaluation(args, down_ratio=down_ratio)
