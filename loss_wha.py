...
class FocalLoss(nn.Module):
  ...
  def forward(self, pred, gt):
      ...
      pos_loss = torch.log(pred + sys.float_info.epsilon) * torch.pow(1 - pred, 2) * pos_inds
...
class LossAll(torch.nn.Module):
    def __init__(self):
        ...
        self.L_cls_theta = OffSmoothL1Loss()
    ...
    def forward(self, pr_decs, gt_batch):
        ...
        ## add
        cls_theta_loss = self.L_cls_theta(pr_decs['angle'], gt_batch['reg_mask'], gt_batch['ind'], gt_batch['angle'])
        ...
        # return loss
        return hm_loss, wh_loss, off_loss, cls_theta_loss
